...
checkRootPermit() {
    [[ $EUID -ne 0 ]] && echo -e "${Error} 请使用 sudo / root 权限运行本脚本" && exit 1
}
...
runBinrayFileHelp() {
    if [ -e ${downPath} ]; then
    if [ $(id -u) -ne 0 ]; then
        sudo setcap cap_net_raw,cap_net_admin+eip ${downPath}
    else
    ...
    echo -e "       关于软件卸载，因为 nexttrace 是绿色版单文件，卸载只需输入命令 (sudo) rm ${downPath} 即可"
    fi
}
